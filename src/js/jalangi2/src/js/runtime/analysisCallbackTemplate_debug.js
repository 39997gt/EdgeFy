...
var constraints_solver = require(process.cwd()+'/src/js/EdgeRefactoring')
var uuid = 0;
J$.mashallingLog ={};
...
var aaa = require(process.cwd()+'/Subject.json').services;
var NUM = 9999;
    ...
    function MyAnalysis() {
        ...
        this.write = function (iid, name, val, lhs, isGlobal, isScriptLocal) {
            ...
            if(90000<parseInt(val) && parseInt(val) <99999) {
                    ...
                    // console.log(colors.magenta("Object Entry Point Identifed"));
                        ...
                        estraverse.traverse(ast, {
                            ...
                            if (node.type =="Identifier" && node.range[0]>=pos && node.range[1]<=end){
                                ...
                                if(parent.parent && parent.parent.type=="VariableDeclarator" && parent.parent.id.name==name) {
                                    // console.log("EEEE", escodegen.generate(parent.parent), parent.parent.type, parent.parent.id.range);
                                    entryfact= format("fp.fact(RWlog(BitVecVal(hashVar[\"{0}\"][\"bin\"],var),BitVecVal({1},val)))",[J$.mashallingLog.entry.filename+":"+parent.parent.id.range[0]+":"+parent.parent.id.range[1],polycrc.crc24(JSON.stringify(val))]);
             ...
             if(!aaa[1].includes("node_modules")&& aaa !=null && aaa.length == 6  && typeof val =="string" && val == "EdgeRefactoring"){
                 console.log(colors.green(J$.mashallingLog));
                 console.log(colors.magenta("@@@@@@@@@@@@@Starting EdgeRefactoring!!!"+constraints_solver(J$.mashallingLog)));

             }
            if(!aaa[1].includes("node_modules")&& aaa !=null && aaa.length == 6  && typeof val =="string" && val == "Restore"){
                    //J$.toRollback = true;
                    console.log(colors.magenta("@@@@@@@@@@@@@Restore"));
                ...
                if(identifers.length==2){
                   text= format("fp.fact(RWlog2(BitVecVal(hashVar[\"{0}\"][\"bin\"],var),BitVecVal(hashVar[\"{1}\"],prop),BitVecVal({2},val)))",[identifers[0],identifers[1], polycrc.crc24(JSON.stringify(val))]);
                }
                    J$.currentOutput = val;
        ...
        //     return estraverse.VisitorOption.Skip;
                    if (node.type =="Identifier" && node.range[0]>=pos && node.range[1]<=end){
                        ...
                        newexitfact= format("fp.fact(RWlog(BitVecVal(hashVar[\"{0}\"][\"bin\"],var),BitVecVal({1},val)))",[J$.mashallingLog.exit.filename+":"+parent.id.range[0]+":"+parent.id.range[1],polycrc.crc24(JSON.stringify(val))]);
                        ...
                        console.log("OOOOOOEEEE");
                        newexitfact= format("fp.fact(RWlog(BitVecVal(hashVar[\"{0}\"][\"bin\"],var),BitVecVal({1},val)))",[J$.mashallingLog.exit.filename+":"+parent.parent.id.range[0]+":"+parent.parent.id.range[1],polycrc.crc24(JSON.stringify(val))]);
                        J$.mashallingLog.exit.value_sid =polycrc.crc24(JSON.stringify(val));
                       }
                    }
                }});
		...
		//if(aaa !=null && aaa.length == 6){
            ...
            if(args[0] && util.inspect(args[0]).includes("EdgeRefactoringStr")){
                var code = fs.readFileSync(aaa[1]).toString();
                var pos = lineColumn(code).toIndex(aaa[2], aaa[3]);
                var end = lineColumn(code).toIndex(aaa[4], aaa[5]);
                ...
                if (node.type =="BlockStatement" && node.range[0]>=pos && node.range[1]<=end && node.body && !J$.mashallingLog.entry){
                    // var findingnode=parent;
                        ...
                        entryfact= format("fp.fact(RWlogs(BitVecVal(hashVar[\"{0}\"][\"bin\"],lineNum),BitVecVal({1},val)))",[J$.mashallingLog.entry.filename+":"+node.body[0].range[0]+":"+node.body[0].range[1],polycrc.crc24("JSRCIStr")]);
                        J$.mashallingLog.entry.value_sid =polycrc.crc24("EdgeRefactoringStr");
                        J$.mashallingLog.entry.rwfacts = entryfact;
                        console.log(colors.magenta("Entry Point Identified"+"  WRITE(|EdgeRefactoringStr|)"+entryfact));
            ...
            }else if(args[0] && util.inspect(args[0]).includes("EdgeRefactoring")){
